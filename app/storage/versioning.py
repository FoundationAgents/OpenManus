"""SQLite-backed versioning engine providing content-addressable storage, snapshots, and rollback support."""
    ...
    def restore_snapshot(self, snapshot_id: str, agent: str = "system", reason: str = "Snapshot restore") -> bool:
        ...
        """
            ...
            for version_id in snapshot.file_versions:
                if not self.rollback_to_version(
                    version_id,
                    agent,
                    f"Restore snapshot {snapshot_id}: {reason}",
                ):
                    success = False
                    logger.error(
                        f"Failed to restore version {version_id} from snapshot {snapshot_id}"
                    )
